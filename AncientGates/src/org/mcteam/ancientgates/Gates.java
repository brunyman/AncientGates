...
public class Gates {
	...
	public static boolean open(final Gate gate) {
		...
		for (final WorldCoord coord : gate.getPortalBlocks().keySet()) {
			...
			//System.out.println("Debug 0 - " + orientation + " - " + material + " - " + coord.getBlock().getData());
			if (orientation == FloodOrientation.VERTICAL1 && material == Material.NETHER_PORTAL || orientation == FloodOrientation.VERTICAL1 && material == Material.LEGACY_PORTAL) {
				//coord.getBlock().setData((byte) 2);
				BlockState bs = coord.getBlock().getState();
				MaterialData md = bs.getData();
				md.setData((byte) 2);
				bs.setRawData((byte) 2);
				bs.setData(md);
				bs.update(true);
				//System.out.println("Debug 1 - " + coord.getBlock().getData());
			}
		}
